...
class SDQA:
    ...
    def __init__(self, activation_fn, is_training, input_shape=48536,
        ...
        self.input1 = tf.placeholder(tf.float32, shape=[None, input_shape])
        self.input2 = tf.placeholder(tf.float32, shape=[None, input_shape])

        with tf.variable_scope("SiameseNet") as scope:
            self.logit1 = self._define_network(self.input1)
            scope.reuse_variables()
            self.logit2 = self._define_network(self.input2)
    ...
    def inference(self):
        return tf.losses.cosine_distance(self.logit1, self.logit2)

    def loss(self, label, margin=1):

        cosine_dist = tf.losses.cosine_distance(self.logit1, self.logit2, reduction=None)
